...
class TransformerBlock(nn.Module):
    def __init__(self, num_attn_layers, global_channels, local_channels, num_centers, num_heads):
        super().__init__()
        channels = global_channels
        ...
        for i in range(num_attn_layers):
            self.attn_layers.append(
                CenterAttentionLayer(global_channels,
                                     num_centers[i], local_channels, num_heads)
            )
